...
// The page's contents arrive on a stream rather than being asked for once: the server sends a
// frame when it has refreshed anything, and one immediately after a connection opens, so a
// reconnected page is current without a request of its own.
function subscribeToPageContent(pageData, onContent) {
    const source = new EventSource(`${pageData.baseURL}/api/pages/${pageData.slug}/stream`);
    let firstFrame = null;
    ...
    // A stream that drops is reopened by the browser; the frame that follows carries the whole
    // of the page, so nothing missed while it was down has to be asked for.
    source.onerror = () => {
        if (firstFrame !== null) {
            const resolve = firstFrame;
            firstFrame = null;
            fetchPageContent(pageData).then(resolve);
        }
    };
...
async function setupPage() {
    ...
    const pageContent = await subscribeToPageContent(pageData, (content) => {
        pageContentElement.innerHTML = content;
        applyPageContentBehaviour(pageContentElement);
    });
...
// What a fresh set of contents needs before it behaves like the rest of the page. The first
// set gets this through setupPage; a set that arrives on the stream gets it through here.
function applyPageContentBehaviour(pageContentElement) {
    setupPopovers();
    setupClocks();
    setupCalendars();
    setupTodos();
