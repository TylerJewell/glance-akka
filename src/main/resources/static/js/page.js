...
// The page's content travels with the document, so the first render needs no second
// round trip. Afterwards the server pushes each new state down an event stream: the
// widgets refresh on their own deadlines, which nobody here can predict, and asking
// again on a timer would show a change no sooner than the timer allowed.
function initialPageContent() {
    const embedded = document.getElementById("initial-page-content");
    return embedded === null ? "" : embedded.innerHTML;
}

function subscribeToPageContent(pageData, onContent) {
    const stream = new EventSource(`${pageData.baseURL}/api/pages/${pageData.slug}/stream`);

    stream.addEventListener("message", (event) => {
        // Every frame carries the whole page, so a frame missed while the connection was
        // down costs nothing: the next one is current state, not a change to apply on top
        // of an assumed one.
        onContent(JSON.parse(event.data).content);
    });

    // EventSource reconnects on its own; this only says so out loud while it is down.
    stream.addEventListener("error", () => {
        document.getElementById("page").setAttribute("data-stream", "reconnecting");
    });

    stream.addEventListener("open", () => {
        document.getElementById("page").setAttribute("data-stream", "open");
    });
...
async function setupPage() {
    ...
    let shownContent = pageContent;
    subscribeToPageContent(pageData, (content) => {
        if (content === shownContent) return;
        shownContent = content;
        pageContentElement.innerHTML = content;
        // The parts of the setup that attach to widget content have to run again over the
        // markup that just replaced it; the rest is page furniture and stays where it is.
